...
function SIFT.diffofg(L)
	...
	for o = 1, D.O do
		...
		for s = 1, S-1 do
			local minus_octave = ArrayMult(L.octave[o][s], -1);
...
function SIFT.extrafine(oframes, octave, smin, thresh, r)
	...
	for p = 1, K do
		...
		for i = 1, 5 do
			...
			Dx = 0.5 *(octave[s][x][y + 1]-octave[s][x][y-1]);
			Dy = 0.5 *(octave[s][x + 1][y]-octave[s][x-1][y]);
			Ds = 0.5 *(octave[s + 1][x][y]-octave[s-1][x][y]);
			-- Compute the Hessian
			Dxx = octave[s][x][y + 1] + octave[s][x][y-1]-2 * octave[s][x][y];
			Dyy = octave[s][x + 1][y] + octave[s][x-1][y]-2 * octave[s][x][y];
			Dss = octave[s + 1][x][y] + octave[s-1][x][y]-2 * octave[s][x][y];

			Dys = 0.25 *(octave[s + 1][x + 1][y] + octave[s-1][x-1][y]-octave[s + 1][x-1][y]-octave[s-1][x + 1][y]);
			Dxy = 0.25 *(octave[s][x + 1][y + 1] + octave[s][x-1][y-1]-octave[s][x-1][y + 1]-octave[s][x + 1][y-1]);
			Dxs = 0.25 *(octave[s + 1][x][y + 1] + octave[s-1][x][y-1]-octave[s-1][x][y + 1]-octave[s + 1][x][y-1]);
			...
			if(c[1][1] > 0.6 and y < N-2) then
				if(c[1][1] < -0.6 and y > 1) then
					dy = 0;
				else
					dy = 1;
				end
			...
			if(c[1][2] > 0.6 and x < M-2) then
				if(c[1][2] < -0.6 and x > 1) then
					dx = 0;
				else
					dx = 1;
				end
		...
		yn <= N-1 and sn >= 0 and sn <= S-1) then
			table.insert(J[1], xn-1)
			table.insert(J[2], yn-1)
			table.insert(J[3], sn-1 + smin)
		end
	end
	return J;
end
...
function SIFT.orientation(oframes, octave, S, smin, sigma0)
	...
	local img, gradient_x, gradient_y;
	-- Compute image gradients
	for si = 1, s_num do
		img = octave[si];
		gradient_x = conv2(img, dx_filter);
		gradient_y = conv2(img, dy_filter);
		for i = 1, M do 
			for j = 1, N do 
				magnitudes[si][i][j] = sqrt(gradient_x[i][j]^2 + gradient_y[i][j]^2);
				if gradient_x[i][j] == 0 and gradient_y[i][j] ~= 0 then
					angles[si][i][j] = mod(gradient_y[i][j] / abs(gradient_y[i][j]) * pi / 2 + 2 * pi, 2 *pi)
	...
	local sp, xp, yp, sigmaw, W, dx, dy, wincoef, bin, theta_max, theta;
	for p = 1, key_num do
		sp = scales[p];
		xp = x_round[p];
		yp = y_round[p];
		sigmaw = win_factor * sigma0 *(2^(sp / S));
		W = floor(3 * sigmaw);
		local histo = zeros(1, NBINS);
		for xs = xp - max(W, xp-1), min((M-2), (xp + W)) do
			for ys = yp - max(W, yp-1), min((N-2), yp + W) do
				dx = xs - x[p];
				dy = ys - y[p];
				if(dx^2 + dy^2 <= W^2 and xs > 0 and xs < M and ys>0 and ys < N and sp + 1 > 0 and sp + 1 < s_num) then 
					wincoef = exp(-(dx^2 + dy^2) /(2 * sigmaw^2));
					bin = floor(NBINS * angles[sp + 1][xs][ys] /(2 * pi) + 1);
					histo[1][bin] = histo[1][bin] + wincoef * magnitudes[sp + 1][xs][ys];
				end
			end
		end 
		theta_max = FindMax2(histo);
		local theta_indx = {};
		for i = 1, 36 do
			if histo[1][i] >= 0.8 * theta_max then
				table.insert(theta_indx, i);
			end
		end
...
function SIFT.descriptor(octave, oframes, sigma0, S, smin, magnif, NBP, NBO)
	...
	for p = 1, key_num do
		...
		descriptor0 = ArrayMult(descriptor0, 1 / descriptor0Norm, output);
		for i = 1, NBP * NBP * NBO do
			if descriptor0[i][1] > 0.2 then
				descriptor0[i][1] = 0.2;
			end
		end
		descriptor0Norm = norm(descriptor0);
		descriptor0 = ArrayMult(descriptor0, 1 / descriptor0Norm, output);
		if p==1 then
...
function SIFT.DO_SIFT(I, O, S)
	...
	for o = 1, scalespace.O do
		...
		for i = 1, oframesLength do
			...
			(oframesPrime[2][i] + red[i]<=(#scalespace.octave[o][1][1]))) then
		...
		for i = 1, #oframesOrientation[1] do
			table.insert(frames[1], 2^(o-1 + scalespace.omin) * oframesOrientation[1][i]);
			table.insert(frames[2], 2^(o-1 + scalespace.omin) * oframesOrientation[2][i]);
			table.insert(frames[3], 2^(o-1 + scalespace.omin) * scalespace.sigma0 * 2^(oframesOrientation[3][i] / scalespace.S));
			table.insert(frames[4], oframesOrientation[4][i]);
...
function SIFT.match(im1, des1, loc1, im2, des2, loc2, distRatio)
	local distRatio = distRatio or 0.75;
	local matched_points_im1 = {};
	local match = {};
	local des2t = transposition(des2);
	local dotprods = zeros(1, #des2);
	local column_output = zeros(1, 128);
	local SortDotprods = {};
	local SortDotprods0 = {};
	local TMP = {};
	local FirstIndex = 1;
	for i = 1, #des1 do 
		TMP[1] = des1[i];
		dotprods = MatrixMultiple(TMP, des2t, dotprods, column_output);
		for j = 1, #dotprods[1] do
			SortDotprods[j] = acos(dotprods[1][j]);
			SortDotprods0[j] = acos(dotprods[1][j]);
		end
	...
	if num == 0 then
		...
		for k = 1, num do
			...
			for j = k + 1, num do
				...
				orien_diff_img2[k][j] = loc2[match[matched_points_im1[k]]][4] - loc2[match[matched_points_im1[j]]][4];
				orien_diff_img2[j][k] = orien_diff_img2[k][j];
			end
		end

		local dis_output = dis_img1[1];
		for i = 1, num do
			dis_img1[i] = ArrayMult(dis_img1[i], 1 / norm(dis_img1[i]), dis_output);
			dis_img2[i] = ArrayMult(dis_img2[i], 1 / norm(dis_img2[i]), dis_output);

			orien_diff_img1[i] = ArrayMult(orien_diff_img1[i], 1 / norm(orien_diff_img1[i]), dis_output);
			orien_diff_img2[i] = ArrayMult(orien_diff_img2[i], 1 / norm(orien_diff_img2[i]), dis_output);
		end
		...
		for i = 1, #des1 do
			if(match[i] > 0) then
				...
				table.insert(locations2[1], loc2[match[i]][1]);
				table.insert(locations2[2], loc2[match[i]][2]);
			end
		end
		
		--Remove the bad location
		local loc_new = ArrayAddArray(locations1, ArrayMult(locations2, -1));
		...
		for i = 1, #locations1[1] do
			if((locations1[1][i]-locations2[1][i])^2 < thre_Rio*threshold_x and (locations1[2][i]-locations2[2][i])^2 < thre_Rio*threshold_y) then
				table.insert(locations1_new[1], locations1[1][i])
				table.insert(locations1_new[2], locations1[2][i])
				table.insert(locations2_new[1], locations2[1][i])
				table.insert(locations2_new[2], locations2[2][i])
			end
		end
		local num_final_match1 = #locations1_new[1];
		return num_final_match1, num_final_match, num, locations1, locations2, locations1_new, locations2_new;
	end
end
local match = SIFT.match;